...
static void render_city_setup(lv_obj_t *screen, const navigation_t *navigation) {
    ...
    if (!show_qr) {
        if (first_setup) {
            ...
            lv_obj_t *action = panel_new(screen, 20, 150, 200, 32, COLOR_TEAL, LV_OPA_20);
            ...
            lv_obj_t *hint = label_new(action, "Press to show setup QR", &lv_font_montserrat_16, COLOR_TEAL);
...
static void render_provisioning(lv_obj_t *screen) {
    ...
    lv_obj_t *title = label_new(screen, failed ? "Connection failed" : network_is_connecting() ? "Connecting to Wi-Fi" : "Connect your phone to", &lv_font_montserrat_20, failed ? COLOR_RED : COLOR_PRIMARY);
