...
static const char settings_page[] =
    ...
    "function loadState(){fetch('/settings-state').then(x=>x.ok?x.json():Promise.reject()).then(x=>{cityState(x);marketState(x)}).catch(()=>{cz.textContent='Could not load city settings.';cc.hidden=true;cf.hidden=false;mz.textContent='Could not load Finnhub key status.'})}"
