...
static const char settings_page[] =
    ...
    "<hr><h2>Market data</h2><p><small>Finnhub key is optional and stays only on this device.</small></p><label>Finnhub API key</label><input id=market_key type=password maxlength=95 autocomplete=off autocapitalize=none spellcheck=false placeholder='Paste your API key'><button type=button id=market_save>Save Finnhub key</button><p id=market_status><small>Loading key status...</small></p>"
    "<script>"
    "let q=document.querySelector('#city_query'),r=document.querySelector('#city_results'),cs=document.querySelector('#city_search'),sv=document.querySelector('#city_save'),cz=document.querySelector('#city_status'),cf=document.querySelector('#city_search_form'),cc=document.querySelector('#city_change'),mk=document.querySelector('#market_key'),ms=document.querySelector('#market_save'),mz=document.querySelector('#market_status');"
    "function showSearch(){cf.hidden=false;cc.hidden=true;q.focus()}"
    "function cityState(x){let configured=!!x.city;cz.textContent=configured?'Current city: '+x.city:'Choose a city to enable local time and weather.';cc.textContent='Change city';cc.hidden=!configured;cf.hidden=configured}"
    "function marketState(x){let configured=!!x.finnhub_key_configured;mz.textContent=configured?'A Finnhub API key is saved ('+x.finnhub_key_mask+'). Enter a new key to replace it.':'No Finnhub API key is saved. Get a key at finnhub.io/register.';ms.textContent=configured?'Replace API key':'Save Finnhub key';mk.placeholder=configured?'Paste a replacement API key':'Paste your API key'}"
    "function loadState(){fetch('/settings-state').then(x=>x.ok?x.json():Promise.reject()).then(x=>{cityState(x);marketState(x)}).catch(()=>{cz.textContent='Could not load city settings.';cc.hidden=true;cf.hidden=false;mz.textContent='Could not load Finnhub key status.'})}function citySearchBusy(busy){cs.disabled=busy;cs.innerHTML=busy?'<span class=spinner></span>Searching...':'Search cities'}function fetchWithTimeout(url){return Promise.race([fetch(url),new Promise((_,reject)=>setTimeout(()=>reject(new Error('timeout')),12000))])}"
    "cc.onclick=showSearch;loadState();"
    "cs.onclick=()=>{let city=q.value.trim();if(!city){cz.textContent='Enter a city name.';return}citySearchBusy(true);cz.textContent='Searching...';fetchWithTimeout('/city-search?query='+encodeURIComponent(city)).then(x=>x.ok?x.json():Promise.reject()).then(items=>{r.textContent='';items.forEach(x=>{let o=document.createElement('option');o.textContent=x.name+' - '+x.timezone;o.value=JSON.stringify(x);r.append(o)});r.hidden=!items.length;sv.hidden=!items.length;cz.textContent=items.length?'Choose the matching city.':'No matching city found.'}).catch(error=>cz.textContent=error&&error.message==='timeout'?'Search timed out. Try again.':'Search unavailable. Check Wi-Fi.').finally(()=>citySearchBusy(false))};"
    "sv.onclick=()=>{if(!r.value)return;let x=JSON.parse(r.value);sv.disabled=true;cz.textContent='Saving city and refreshing weather...';fetch('/city-save',{method:'POST',body:new URLSearchParams(x)}).then(y=>y.ok?y.text():Promise.reject()).then(t=>{cityState({city:x.name});cz.textContent=t;cf.hidden=true}).catch(()=>cz.textContent='Could not save city.').finally(()=>sv.disabled=false)};"
    "ms.onclick=()=>{let key=mk.value.trim();if(!key){mz.textContent='Enter an API key to save it.';return}ms.disabled=true;mz.textContent='Saving key...';fetch('/market-key',{method:'POST',body:new URLSearchParams({api_key:key})}).then(x=>x.ok?x.text():Promise.reject()).then(t=>{mk.value='';mz.textContent=t;loadState()}).catch(()=>mz.textContent='Could not save key.').finally(()=>ms.disabled=false)}</script></body></html>";
...
static esp_err_t settings_state_get_handler(httpd_req_t *req) {
    ...
    const bool has_finnhub_key = settings_has_market_key(active_settings);
    char finnhub_key_mask[12] = "";
    if (has_finnhub_key) {
        const size_t key_length = strlen(active_settings->finnhub_api_key);
        if (key_length > 4) snprintf(finnhub_key_mask, sizeof(finnhub_key_mask), "****%s", active_settings->finnhub_api_key + key_length - 4);
        else strlcpy(finnhub_key_mask, "****", sizeof(finnhub_key_mask));
    }
    cJSON_AddBoolToObject(state, "finnhub_key_configured", has_finnhub_key);
    cJSON_AddStringToObject(state, "finnhub_key_mask", finnhub_key_mask);
