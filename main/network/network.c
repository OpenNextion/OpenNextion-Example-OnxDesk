...
static const char settings_page[] =
    ...
    "function cityState(x){let configured=!!x.city;cz.textContent=configured?'Current city: '+x.city:'Choose a city to enable local time and weather.';cc.textContent=configured?'Change city':'Set up city';cc.hidden=false;cf.hidden=configured}"
    ...
    "function loadState(){fetch('/settings-state').then(x=>x.ok?x.json():Promise.reject()).then(x=>{cityState(x);marketState(x)}).catch(()=>{cz.textContent='Could not load city settings.';cc.textContent='Set up city';cc.hidden=false;mz.textContent='Could not load Finnhub key status.'})}"
    "cc.onclick=showSearch;loadState();"
    "cs.onclick=()=>{let city=q.value.trim();if(!city){cz.textContent='Enter a city name.';return}cs.disabled=true;cz.textContent='Searching...';fetch('/city-search?query='+encodeURIComponent(city)).then(x=>x.ok?x.json():Promise.reject()).then(items=>{r.textContent='';items.forEach(x=>{let o=document.createElement('option');o.textContent=x.name+' - '+x.timezone;o.value=JSON.stringify(x);r.append(o)});r.hidden=!items.length;sv.hidden=!items.length;cz.textContent=items.length?'Choose the matching city.':'No matching city found.'}).catch(()=>cz.textContent='Search unavailable. Check Wi-Fi.').finally(()=>cs.disabled=false)};"
